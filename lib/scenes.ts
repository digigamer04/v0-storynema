...
// Añadir función para eliminar escenas
...
// Añadir función para reordenar escenas con actualización inmediata
export async function reorderScenesInstant(projectId: string, sceneIds: string[]) {
  try {
    const supabase = createClientComponentClient<Database>()

    console.log("Reordenando escenas instantáneamente:", {
      projectId,
      sceneCount: sceneIds.length,
    })
    ...
    console.log("Reordenamiento instantáneo completado con éxito")
    return true
  } catch (error: any) {
    console.error("Error en reorderScenesInstant:", error)
    throw new Error(`Error al reordenar las escenas: ${error.message}`)
  }
}
